...
func GenerateToken(loginInfo *request.UserLogin, UserId string, expiredSeconds int) (string, error) {
	...
	logger.Recorder.Info("Token will be expired at " + time.Unix(expireAt, 0).String())
...
func ParseToken(tokenString string) (*JwtPayload, error) {
	...
	if err != nil {
		logger.Recorder.Warn("Error user token: ", err.Error())
		return nil, err
	}
	claims, ok := token.Claims.(*MyCustomClaims)
	if ok && token.Valid {
		logger.Recorder.Info(fmt.Sprintf("%v %v", claims.UserID, claims.StandardClaims.ExpiresAt))
		logger.Recorder.Info(fmt.Sprintf("Token will be expired at ", time.Unix(claims.StandardClaims.ExpiresAt, 0)))
		...
		logger.Recorder.Info("[JWT]" + err.Error())
